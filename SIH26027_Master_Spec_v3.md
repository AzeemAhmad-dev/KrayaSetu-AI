...
**Warm-start note:** hint `x_j` once per job, not once per leg — every leg shares the same literal, so hinting them differently is a contradiction the solver just discarded. Same lesson as the block-vs-`x_i` hinting note in 3.3(4).
