...
def build_gantt_tasks(request: ScheduleRequest, sel_wps, tr_schedules, start_dt: datetime, horizon_minutes: int):
    tasks = []
    # 1. Parents for block sections
    for bs in request.block_sections:
        tasks.append(GanttTask(
            id=bs.block_section_id,
            text=f"Block {bs.block_section_id} ({bs.name or 'Track'})",
            start_date=start_dt.strftime("%Y-%m-%d %H:%M"),
            end_date=(start_dt + timedelta(minutes=horizon_minutes)).strftime("%Y-%m-%d %H:%M"),
            parent="",
            type="project",
            priority_color="",
            conflict=False
        ))
    # 2. Selected Work Packages
    for wp in sel_wps:
        p_color = "red" if wp.priority_score >= 4.0 else ("orange" if wp.priority_score >= 3.0 else ("yellow" if wp.priority_score >= 2.0 else "green"))
        bs_parent = wp.block_sections_used[0] if wp.block_sections_used else (request.block_sections[0].block_section_id if request.block_sections else "")
        s_date = wp.assigned_start[:16].replace("T", " ")
        e_date = wp.assigned_end[:16].replace("T", " ")
        tasks.append(GanttTask(
            id=wp.work_id,
            text=f"{wp.work_id} (P={wp.priority_score})",
            start_date=s_date,
            end_date=e_date,
            parent=bs_parent,
            type="maintenance",
    ...
    # 3. Traveling Job Schedules
    for tr in tr_schedules:
        for idx, leg in enumerate(tr.legs):
            s_date = leg.start[:16].replace("T", " ")
            e_date = leg.end[:16].replace("T", " ")
            tasks.append(GanttTask(
                id=f"{tr.work_id}-leg-{idx+1}",
                text=f"{tr.work_id} Leg {idx+1}",
                start_date=s_date,
                end_date=e_date,
                parent=leg.block_section_id,
                type="maintenance",
...
def solve_plan(request: ScheduleRequest) -> ScheduleResponse:
    ...
    def generate_fallback_response():
        ...
        return ScheduleResponse(
            ...
            gantt_tasks=build_gantt_tasks(request, sel_wps, tr_schedules, start_dt, HORIZON),
            conflicts=[],
